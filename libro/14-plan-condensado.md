# 14 · Plan del libro 1 — extensión, economía y alcance

> ## El libro 1 tiene **14 capítulos**, ~34.000 palabras y ~170 páginas.
>
> Índice completo en el §4 de este archivo; cada capítulo desarrollado en
> [15-biblia-de-capitulos.md](15-biblia-de-capitulos.md).

> **Este archivo sustituye al plan de extensión de los archivos 03, 08 y 09.** El resto del dossier
> sigue siendo válido como **cantera**: la investigación, la evidencia y los casos no cambian, solo
> cambia cuánto de todo eso entra en el libro 1.

---

## 1. La economía, corregida

| Escenario | Precio con IVA | Págs. | Impresión | **Regalía** | Amortizar 900–2.400 € |
|---|---|---|---|---|---|
| A · Compacto | 17,95 € | 140 | 2,43 € | 7,93 € | 114–303 ej. |
| **B · Vigente** | **17,95 €** | **170** | **2,79 €** | **7,57 €** | **119–317 ej.** |
| C · Precio de entrada | 15,95 € | 170 | 2,79 € | 6,41 € | 140–374 ej. |
| ~~D · Plan viejo~~ | 15,99 € | 240 | 3,63 € | 5,59 € | 161–429 ej. |

**El argumento que decide.** De A a B se pierden **36 céntimos por ejemplar** y se ganan treinta
páginas. Optimizar la regalía unitaria a costa de amputar contenido es optimizar la variable
equivocada:

> **La página cuesta poco. La página innecesaria cuesta atención.** Se corta por valor editorial,
> nunca para perseguir un escalón de impresión.

**Y el precio no se decide por regalía.** Entre B y C hay 1,16 € por ejemplar, pero una diferencia
de conversión del 20 % los borra. **15,95 € frente a 17,95 € es un test del prototipo**, no una
decisión de hoja de cálculo. Lo único cierto: fuera de la banda del 70 % se pierde la mitad, y el
eBook debe quedar ≥20 % por debajo del impreso.

**El libro corto y caro gana por goleada: +42% de regalía por ejemplar escribiendo menos de la
mitad de palabras.** Y para amortizar los 900–2.400 € de producción hacen falta **114–303
ejemplares** en vez de 161–429.

**Dos hallazgos técnicos de la verificación:**

1. **No hay acantilado en las 110 páginas.** KDP cobra **tarifa plana** hasta ~108 páginas y a
   partir de 110 pasa a la fórmula `0,75 € + 0,012 €/página`. La diferencia entre 108 y 110
   páginas son céntimos. **No hay que contorsionar el libro para caer en un lado**: la extensión
   es una decisión editorial, no de coste de impresión.
2. **El precio importa 20 veces más que las páginas.** Cada página añadida cuesta 1,2 céntimos;
   cada euro de precio añade 60 céntimos de regalía. Escribir 100 páginas más para justificar 2 €
   más de precio es un mal negocio.

## 2. Formato

**~170 páginas en 5,5"×8,5".** El 6"×9" de 110 páginas quedó descartado por dos motivos:

- El **precio por página** baja de 0,163 € a 0,128 €, que es la objeción real de un comprador de
  Amazon cuando ve «110 páginas · 17,95 €».
- Un 5,5"×8,5" de 140 páginas **parece un libro**; un 6"×9" de 110 parece un folleto caro.
- El formato más pequeño es el estándar de manual práctico y de libro de bolsillo profesional, que
  es exactamente la categoría mental que nos conviene.

**Y el posicionamiento que hace defendible el precio:** esto no se vende como «libro», se vende
como **manual de trabajo**. Un libro de 140 páginas a 17,95 € es caro. Un manual con expediente
rellenable de 140 páginas a 17,95 € es normal — la gente paga por herramientas lo que no paga por
prosa.

## 3. El principio editorial que resuelve tu miedo a la densidad

Aquí está la decisión más importante de todo el replanteamiento, y es contraintuitiva:

> **Condensar no es comprimir cada capítulo. Es eliminar temas enteros.**

Meter una arquitectura de veintitantos capítulos en 170 páginas produce un ladrillo denso que
nadie termina. Si bajamos a **14 capítulos y dejamos que cada uno respire**,
el resultado es lo contrario: un libro rápido, con aire, con una idea por capítulo y una acción al
final de cada uno.

**Presupuesto real:**

| Bloque | Páginas | Palabras |
|---|---|---|
| Preliminares (portada, créditos, índice, nota del autor) | 8 | — |
| Prólogo | 4 | 900 |
| **14 capítulos × ~2.200 palabras** | 128 | 30.800 |
| Epílogo | 3 | 700 |
| Apéndice: el expediente (8 plantillas) | 16 | — |
| Notas y fuentes | 9 | 1.600 |
| Cierre y llamada al libro 2 | 2 | 200 |
| **Total** | **~170** | **~34.200** |

**34.000 palabras frente a las 59.000 del plan anterior.** Menos de la mitad, más rentable, y
—esto es lo importante— **mejor producto para este lector**: alguien que trabaja ocho horas y
quiere respuestas, no un tratado.

**Y tres reglas para que no quede denso:**

1. **Una sola idea dominante por capítulo.** Si hay dos tesis, son dos capítulos o sobra una.
2. **Un solo estudio en el cuerpo del texto por capítulo**, contado como historia. Los otros
   dieciocho de la bibliografía viven en las notas del final, para quien quiera tirar del hilo.
   Así se conserva la credibilidad sin taponar la página.
3. **Aire deliberado.** Cada capítulo termina con media página en blanco de resumen y acción. En un
   libro de 140 páginas, el blanco no es desperdicio: es lo que hace que se lea de un tirón.

## 4. Los catorce capítulos

Criterio de selección: qué es **único frente a la competencia**, qué tiene **evidencia fuerte**,
qué es **accionable el lunes** y qué **engancha**. Todo lo que no puntúa alto en al menos tres, se
va al libro 2 o desaparece.

| # | Capítulo | Movimiento | Idea única |
|---|---|---|---|
| **1** | El expediente que no has visto | **E** | Existe una sala donde se habla de ti. Y llevas años mejorando el sistema equivocado |
| **2** | Cinco escaleras, no una | **E** | Solo una de las cinco tiene cola en la entrada, y no es la mejor: es la única señalizada |
| **3** | Pruébalo sin jugártela | **E** | No hace falta apostar la carrera para saber si te gusta. Hacen falta ocho semanas |
| **4** | Cómo se decide de verdad | **S** | Las tres puertas: capacidad, oportunidad y patrocinio. Solo una depende de ti |
| **5** | Tu inventario en una página | **S** | Un curso no es capacidad: es un intento de capacidad |
| **6** | Trabajar bien no basta | **C** | El mérito invisible no existe. Y no todo el trabajo cuenta igual |
| **7** | Haz una prueba del nivel siguiente | **C** | El marrón que nadie quiere es la única oportunidad sin competencia |
| **8** | Caer bien no es un defecto | **A** | La adulación te gana a quien se la haces y te cuesta a todos los que la ven |
| **9** | Mentor no: padrino | **A** | Un mentor te habla a ti; un padrino habla de ti cuando no estás |
| **10** | La red que decide sobre ti | **A** | Las oportunidades llegan por los conocidos, no por los íntimos |
| **11** | Haz de jefe antes de serlo | **L** | Presentar el trabajo de otros **es** el trabajo. La prueba del autor en la sala |
| **12** | Pide | **L** | Dice que sí mucha más gente de la que crees |
| **13** | Que no te lo hagan a ti | **A** | Ocho señales de que el problema no eres tú |
| **14** | Acepta con condiciones | **A** | La reversibilidad no se hereda: se negocia, y se negocia antes |

**ESCALA vuelve a ordenar el libro**, en vez de quedarse en la portada: Elige (1–3) · Sitúate
(4–5) · Construye (6–7) · Amplifica (8–10) · Lidera (11–12) · Asegura (13–14).


**Comprobaciones de coherencia:**
- El bucle del capítulo 1 («¿qué pone en mi ficha?») cierra en el 3.
- La promesa de reversibilidad del prólogo y el capítulo 1 cierra en el **12**. Por eso el 12 se
  queda en el libro 1 aunque hable de después del ascenso: sin él, el libro promete algo que no
  cumple.
- Las cinco rutas (mando, experta, lateral, salida, permanencia) **no son un capítulo**: son dos
  páginas dentro del capítulo 1. Mantienen la honestidad sin gastar ocho páginas en un libro cuya
  promesa es ascender.

## 5. Qué se va al libro 2

La partición natural es limpia y comercialmente evidente:

> **Libro 1 — hasta que te lo dan.**
> **Libro 2 — desde que te lo dan.**

| Material | Origen |
|---|---|
| Los primeros 90 días, el punto de equilibrio y el marco STARS | Cap. 22 actual |
| Lo que hay que soltar en cada peldaño, y el duelo por el trabajo que ya no harás | Caps. 21–22 |
| Dirigir a quienes eran tus iguales | Cap. 20 |
| Delegar cuando crees que lo harías mejor tú | Cap. 22 |
| Feedback: por qué falla un tercio de las veces y cómo darlo | Cap. 19 |
| Dar sin que te vacíen: los generosos en el fondo y en la cima | Cap. 19 |
| Los bucles, las demoras y las trampas del sistema | Archivo 12, §4–5 |
| Cuando sale mal: volver atrás con dignidad | Cap. 23 |
| Los cuatro finales, desarrollados | Cap. 23 |

**Por qué es un buen libro 2 y no un descarte:** el lector del libro 1 que consigue el ascenso
*se convierte* en el lector del libro 2 unos meses después. Es el mismo comprador, con un problema
nuevo y más urgente. Y el segundo público —el mando recién nombrado que se está ahogando— ya
estaba identificado como lector secundario desde la primera versión del dossier.

**Título de trabajo del libro 2:** *Los primeros noventa días* está cogido (Watkins). Candidatos:
**«Ahora mandas»** · **«El primer equipo»** · **«ESCALA 2 · Sostenerlo»**.

**Regla de venta cruzada:** las dos últimas páginas del libro 1 presentan el libro 2 sin
mendigar — planteando el problema que el lector va a tener dentro de tres meses.

## 6. Qué desaparece del todo

Sin lástima. Nada de esto sobrevive al filtro de «valor por página»:

- **La deconstrucción de los bestsellers** (archivo 07). Es material de dossier, para ti; no le
  interesa al lector.
- **La discusión del 70‑20‑10.** Se usa De Pater directamente y no se menciona el modelo popular.
- **La teoría de sistemas extensa.** Sobreviven los tres sistemas (dos páginas en el capítulo 1) y
  las cinco reservas (capítulo 3). Los bucles, las demoras y las trampas se van al libro 2.
- **La cuota por sectores como estructura.** Se convierte simplemente en: los ejemplos se reparten
  entre almacén, tienda, hostelería, industria, sanidad y oficina. Sin recuadros dedicados.
- **El recuadro del sector público.** Pasa a ser una nota de dos párrafos en el capítulo 2.
- **Los apéndices B, C y D** (guiones extensos, glosario, lecturas). El glosario se integra en el
  texto la primera vez que aparece cada término; los guiones van dentro de sus capítulos.
- **El expediente pasa de 14 plantillas a 8.** Las que se quedan: diagnóstico, mapa de decisión,
  brechas, registro SARA, carta de proyecto, mapa de relaciones, guion de la conversación, lista
  de aceptación.

## 7. Consecuencias en el resto del dossier

| Archivo | Estado |
|---|---|
| 01 Concepto | **Válido.** El posicionamiento cambia a «manual de trabajo», no a «libro» |
| 02 Arquitectura | **Válido en lo conceptual.** Los tres actos pasan a ser 1–2 / 3–10 / 11–12 |
| 03 Esqueleto | **ANEXO.** Mapa del libro 2. No es el índice del libro 1 |
| 04 Bibliografía | **Válido y necesario.** Ahora es más importante: 12 estudios en el cuerpo, el resto en notas |
| 05 Casos | **Válido.** El caso ancla se trocea en 4 entregas en vez de 6 |
| 06 Enganche | **Válido.** La plantilla de capítulo se recorta a 8 bloques |
| 07 Bestsellers | **ANEXO.** Solo dossier. No entra en el libro |
| 08 KDP | **Actualizar** con la economía de este archivo |
| 09 Plan de trabajo | **Se acorta:** la fase de manuscrito baja de 13 a 7 semanas |
| 10 Piezas redactadas | **Válido.** El tono no cambia |
| 11 Expediente | **Recortar** de 14 a 8 plantillas |
| 12 Sistema | **Parcialmente al libro 2.** Sobreviven §1 y §3 |
| 13 Influencia y defensa | **Válido.** Alimenta los capítulos 6, 9 y 11, que son tres de los doce |

## 8. Nuevo calendario

Escribir ~34.000 palabras en vez de 59.000 cambia el proyecto de escala:

| Fase | Antes | Ahora |
|---|---|---|
| Verdad del autor | 3 días | 3 días |
| Validación de mercado | 2 sem. | 2 sem. |
| Campo (entrevistas) | 5 sem. | **3 sem.** — 20 entrevistas bastan para 14 capítulos |
| Prototipo | 3 sem. | 2 sem. |
| Manuscrito | 13 sem. | **8 sem.** a 2 capítulos por semana |
| Reescritura | 5 sem. | 4 sem. |
| Producción | 3 sem. | 3 sem. |
| Lanzamiento | 8 sem. | 8 sem. |
| **Total** | 9,5 meses | **~7,5 meses** |

Con un trabajo a jornada completa: **8 o 9 meses** en vez de 11–13.

## 9. Los riesgos reales de este plan, y qué hacer

| Riesgo | Gravedad | Mitigación |
|---|---|---|
| **«17,95 € por 140 páginas es caro»** | Alta — es la objeción principal | Posicionar como manual con herramientas, no como ensayo. Contenido A+ enseñando el interior y las plantillas. Formato 5,5"×8,5". Y probar 15,95 € frente a 17,95 € en el lanzamiento |
| **Al condensar se pierde la evidencia, que es el diferenciador** | Alta | La regla del archivo: **un estudio en el cuerpo por capítulo, el resto en notas**. Las notas finales son parte del argumento de venta, no relleno |
| **Queda un libro genérico de consejos** | Media | Cada capítulo mantiene su escena de apertura y su contraejemplo. Si un capítulo no cabe con escena, el problema es el capítulo, no la extensión |
| **El lector siente que le falta la segunda mitad** | Media | Es una función, no un fallo — siempre que el libro 1 sea **completo en su promesa**: llegar al ascenso. Por eso el capítulo 12 no se puede mover al libro 2 |
| **El libro 2 no llega nunca** | Media | No anunciarlo como «próximamente» con fecha. Presentarlo como el problema siguiente, sin promesa de calendario |
| **Menos páginas, menos palabras clave indexadas** | Baja | El indexado real viene de título, subtítulo, categorías y descripción, no del cuerpo |

## 10. Decisión

**Vigente:** **~170 páginas en 5,5"×8,5", 14 capítulos, ~34.000 palabras**, y el precio decidido
por prueba entre 15,95 € y 17,95 €. Se escribe en la mitad de tiempo que el plan original, deja un
segundo libro ya investigado, y no amputa ni la elección de ruta ni la prueba reversible.

**Los dos errores que este archivo corrige, y que eran míos:**
1. Optimizar la regalía por ejemplar recortando contenido. Treinta páginas cuestan 36 céntimos.
2. Fijar el precio en una hoja de cálculo. Se fija con un test de prototipo.

**Y lo que sigue en pie:** catorce capítulos que respiran, no veintitantos apretados.
